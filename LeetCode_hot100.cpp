...
/*
假设入环前长度为a
fast、slow相遇为S点，入环点到S点的距离为b，S点到入环点的距离为c
slow指针进入环后，又走了b的距离与fast相遇，此时fast指针以及走了n圈
fast = a + n(b + c) + b
slow = a + b
fast的速度为slow的两倍, 2a + 2b = a + n(b + c) + b, 即a = (n-1)b + nc = (n - 1)(b + c) + c
从相遇点到入环点的距离加上 n−1 圈的环长，恰好等于从链表头部到入环点的距离
即一个指针从S点出发、一个指针从head出发，最后会在入环点相遇
*/
class Solution
{
public:
